...
document.addEventListener('DOMContentLoaded', function () {
    document.querySelectorAll('form[data-action="suspend_employee"]').forEach(function (form) {
        // Weka flag ili kuzuia binding ya sapta-action-form
        form.setAttribute('data-suspend-handler', 'true');
        ...
        form.addEventListener('submit', function (e) {
            ...
            Swal.fire({
                ...
                inputPlaceholder: 'Enter the reason for suspending this employee...',
                inputAttributes: {
                    'required': 'required',
                    'maxlength': 1000,
                    'rows': 4,
                ...
                inputValidator: (value) => {
                    if (!value || value.trim() === '') {
                        return 'Reason is required to continue!';
                    }
                    if (value.trim().length < 5) {
                        return 'Reason is too short — please write more than 5 characters.';
                    }
                }
            }).then((result) => {
...
/* ============================================================
   FINAL GUARD — Zuia sapta-action-form kabisa kwa suspend_employee
   ============================================================ */
document.addEventListener('DOMContentLoaded', function () {
    // Ondoa event listeners zote za awali kwa suspend_employee forms
    document.querySelectorAll('form[data-action="suspend_employee"]').forEach(function (form) {
        var newForm = form.cloneNode(true);
        form.parentNode.replaceChild(newForm, form);
    });
    
    // Sasa weka handler yetu pekee
    document.querySelectorAll('form[data-action="suspend_employee"]').forEach(function (form) {
        form.addEventListener('submit', function (e) {
            e.preventDefault();
            e.stopPropagation();
