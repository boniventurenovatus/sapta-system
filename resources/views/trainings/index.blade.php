    ...
    <div class="tr-card">
        ...
        <div class="tr-card-body">
            <form method="GET" action="{{ route('trainings.index') }}" class="tr-filter">
                ...
                <div class="tr-form-group"><label>Department</label><div class="tr-input-wrap"><i class="fas fa-building"></i><select name="department_id" class="tr-input"><option value="">All Departments</option>@foreach($departments as $d)<option value="{{ $d->id }}" @selected(request('department_id') == $d->id)>{{ $d->name }}</option>@endforeach</select></div></div>
