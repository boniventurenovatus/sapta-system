    ...
    <div class="rc-card">
        ...
        <div class="rc-card-body">
            <form method="GET" action="{{ route('recruitment.index') }}" class="rc-filter">
                ...
                <div class="rc-form-group"><label>Department</label><div class="rc-input-wrap"><i class="fas fa-building"></i><select name="department_id" class="rc-input"><option value="">All Departments</option>@foreach(($departments ?? \App\Models\Department::where("is_active", true)->orderBy("name")->get()) as $d)<option value="{{ $d->id }}" @selected(request('department_id') == $d->id)>{{ $d->name }}</option>@endforeach</select></div></div>
