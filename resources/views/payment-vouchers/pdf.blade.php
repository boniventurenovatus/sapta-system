...
<body>
    ...
    <div class="header">
        ...
        <p>{{ $pv->organization?->name ?? 'SAPTA Technologies Limited' }}</p>
    </div>
    ...
    <div class="voucher-info">
        <div class="col">
            <div class="row">
                ...
                <span>{{ $pv->voucher_number ?? 'N/A' }}</span>
            </div>
            <div class="row">
                <span class="label">Date:</span>
                <span>{{ $pv->payment_date ? \Carbon\Carbon::parse($pv->payment_date)->format('d M Y') : date('d M Y') }}</span>
            </div>
            <div class="row">
                <span class="label">Department:</span>
                <span>{{ $pv->department?->name ?? 'N/A' }}</span>
            </div>
            <div class="row">
                <span class="label">Project:</span>
                <span>{{ $pv->project?->name ?? 'N/A' }}</span>
            </div>
        </div>
        <div class="col">
            <div class="row">
                <span class="label">Payee:</span>
                <span>{{ $pv->payee_name ?? 'N/A' }}</span>
            </div>
            <div class="row">
                <span class="label">Status:</span>
                <span>{{ ucfirst($pv->status ?? 'draft') }}</span>
            </div>
            <div class="row">
                <span class="label">Currency:</span>
                <span>{{ $pv->currency ?? 'TZS' }}</span>
            </div>
            <div class="row">
                <span class="label">Payment Method:</span>
                <span>{{ ucfirst($pv->payment_method ?? 'N/A') }}</span>
            </div>
        </div>
    </div>
    ...
    <table class="items">
        ...
        <tbody>
            @forelse($pv->items ?? [] as $index => $item)
                <tr>
                    <td>{{ $index + 1 }}</td>
    ...
    <div class="totals">
        <div class="row grand-total">
            ...
            <div class="value">{{ $pv->currency ?? 'TZS' }} {{ number_format($pv->amount ?? 0, 2) }}</div>
        </div>
    </div>

    @if($pv->amount_in_words)
    ...
    <div class="signatures">
        <div class="col">
            ...
            <div class="name">{{ $pv->preparedBy?->username ?? '_______________' }}</div>
        </div>
        <div class="col">
            <div class="line">Checked By</div>
            <div class="name">{{ $pv->checkedBy?->username ?? '_______________' }}</div>
        </div>
        <div class="col">
            <div class="line">Approved By</div>
            <div class="name">{{ $pv->approvedBy?->username ?? '_______________' }}</div>
        </div>
        <div class="col">
            <div class="line">Authorized By</div>
            <div class="name">{{ $pv->authorizedBy?->username ?? '_______________' }}</div>
        </div>
    </div>
