...
<nav class="sapta-sidebar-nav">
    ...
    <div class="sapta-nav-section">
        <div class="sapta-nav-section-title">OVERSIGHT</div>
    ...
    <div class="sapta-nav-section">
        ...
        <a href="{{ route('notifications.index') }}" class="sapta-nav-item {{ request()->is('notifications*') ? 'active' : '' }}">
            ...
            @auth
                @if(\Schema::hasTable('notifications') && \Schema::hasTable('notifications') && \Schema::hasTable('notifications') && auth()->user()->unreadNotifications->count() > 0)
