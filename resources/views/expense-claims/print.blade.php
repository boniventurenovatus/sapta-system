...
<head>
    <meta charset="UTF-8">
    <title>Print Expense Claim {{ $claim->claim_number }}</title>
    <style>
...
<body>
    ...
    {{-- TOOLBAR --}}
    <div class="toolbar">
        <a href="{{ route('expense-claims.show', $claim->id) }}" class="btn-back">← Back</a>
        ...
        <div class="info-grid">
            ...
            <div class="info-col">
                <div class="info-box">
                    ...
                    <div class="info-row"><span class="info-label">Title:</span><span class="info-value">{{ $claim->title }}</span></div>
                    <div class="info-row"><span class="info-label">Category:</span><span class="info-value">{{ ucfirst($claim->category) }}</span></div>
                    <div class="info-row"><span class="info-label">Date:</span><span class="info-value">{{ $claim->expense_date?->format('d M Y') }}</span></div>
                    <div class="info-row"><span class="info-label">Method:</span><span class="info-value">{{ ucwords(str_replace('_', ' ', $claim->payment_method)) }}</span></div>
                </div>
            </div>
        </div>

        <div class="section-title">Description</div>
        <p style="margin-bottom: 15px;">{{ $claim->description }}</p>

        @if($claim->return_reason)
            <div class="section-title" style="color: #dc2626;">Rejection Reason</div>
            <p style="margin-bottom: 15px; color: #991b1b;">{{ $claim->return_reason }}</p>
        @endif

        <div class="section-title">Approval Information</div>
        <table style="width: 100%; border-collapse: collapse;">
            <tr>
                <th style="background: #f1f5f9; padding: 8px; text-align: left; border: 1px solid #e2e8f0; font-size: 9px;">Prepared By</th>
            ...
            <tr>
                <td style="padding: 8px; border: 1px solid #e2e8f0;">{{ $creator?->username ?? 'N/A' }}</td>
                <td style="padding: 8px; border: 1px solid #e2e8f0;">{{ $approver?->username ?? 'Pending' }}</td>
                <td style="padding: 8px; border: 1px solid #e2e8f0;">{{ $claim->approved_at ? \Carbon\Carbon::parse($claim->approved_at)->format('d M Y H:i') : 'Pending' }}</td>
            </tr>
        </table>
