...
class NotificationService
{
     ...
     * Format: [jina]@sapta2024[X]
     * Ambapo [X] = herufi moja kutoka SAPTA (kubwa au ndogo)
     */
    ...
    /**
     * Tuma internal message kwa user
     */
    public static function sendInternalMessage(User $user, string $subject, string $message): bool
    {
        try {
            ...
            Log::info('INTERNAL MESSAGE: ' . $subject, ['user_id' => $user->id, 'message' => $message]);
            return true;
        } catch (\Exception $e) {
    ...
    public static function sendCredentials(User $user, string $plainPassword): array
    {
        ...
        $expiryDays = config('sapta.credentials_expiry_days', 7);
        ...
        $message .= "- Credentials hizi zita-expire baada ya siku {$expiryDays}\n";
