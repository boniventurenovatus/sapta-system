...
class DashboardService
{
    ...
    public function hrData(): array
    {
        return [
            ...
            'recent_leaves' => LeaveRequest::with('employee')->orderBy('created_at', 'desc')->take(5)->get(),
            'pending_leaves_list' => LeaveRequest::where('status', 'pending')->with('employee')->take(5)->get(),
        ];
    }
    ...
    public function financeData(): array
    {
        return [
            'kpis' => [
                'total_budget'      => \Schema::hasTable('budgets') ? Budget::sum('allocated_amount') : 0,
                'total_spent'       => \Schema::hasTable('budgets') ? Budget::sum('spent_amount') : 0,
                'total_remaining'   => \Schema::hasTable('budgets') ? (Budget::sum('allocated_amount') - Budget::sum('spent_amount')) : 0,
            ...
            'recent_vouchers' => PaymentVoucher::with('creator')->orderBy('created_at', 'desc')->take(5)->get(),
    ...
    public function programData(): array
    {
        return [
            ...
            'recent_projects' => Project::with('projectManager')->orderBy('created_at', 'desc')->take(5)->get(),
            'recent_tasks' => Task::with(['project', 'assignee'])->orderBy('created_at', 'desc')->take(5)->get(),
        ];
    }
    ...
    public function managerData(): array
    {
        return [
            ...
            'recent_tasks' => Task::with(['project', 'assignee'])->orderBy('created_at', 'desc')->take(5)->get(),
            'pending_leaves' => LeaveRequest::where('status', 'pending')->with('employee')->take(5)->get(),
        ];
    }
    ...
    public function staffData($userId): array
    {
        ...
        return [
            'kpis' => [
                ...
                'completed_tasks' => Task::where('assigned_to', $employeeId)->where('status', 'done')->count(),
                'pending_leaves'  => LeaveRequest::where('employee_id', $employeeId)->where('status', 'pending')->count(),
                'my_attendance'   => \App\Models\Attendance::where('employee_id', $employeeId)->whereMonth('created_at', now()->month)->count(),
            ],
            'charts' => [
                'my_tasks_by_status' => $this->myTasksByStatus($employeeId),
                'my_attendance_chart' => $this->myAttendanceChart($employeeId),
            ],
            'recent_tasks' => Task::where('assigned_to', $employeeId)->orderBy('created_at', 'desc')->take(5)->get(),
            'my_leaves' => LeaveRequest::where('employee_id', $employeeId)->orderBy('created_at', 'desc')->take(5)->get(),
        ];
    }
    ...
    {
        return PaymentVoucher::whereIn('status', ['pending_approval', 'submitted'])->count()
    ...
    private function monthlyRevenue(): array
    {
        ...
        for ($i = 11; $i >= 0; $i--) {
            ...
            $data[] = (float) PaymentVoucher::whereYear('created_at', $month->year)
    ...
    {
        $data = Task::where('assigned_to', $employeeId)
    ...
    private function myAttendanceChart($employeeId): array
    {
        ...
        for ($i = 5; $i >= 0; $i--) {
            ...
            $present[] = \App\Models\Attendance::where('employee_id', $employeeId)
                ->whereYear('created_at', $month->year)
                ->whereMonth('created_at', $month->month)
                ->where('status', 'present')
                ->count();

            $absent[] = \App\Models\Attendance::where('employee_id', $employeeId)
                ->whereYear('created_at', $month->year)
                ->whereMonth('created_at', $month->month)
                ->where('status', 'absent')
                ->count();
    ...
    {
        $data = PaymentVoucher::select('status', DB::raw('count(*) as count'))->groupBy('status')->get();
    ...
    {
        $data = LeaveRequest::select('leave_type', DB::raw('count(*) as count'))->groupBy('leave_type')->get();
    ...
    {
        $projects = Project::orderBy('progress', 'desc')->take(8)->get();
    ...
    {
        $projects = Project::orderBy('budget', 'desc')->take(8)->get();
    ...
    {
        return Project::orderBy('budget', 'desc')->take(5)->get()->toArray();
    }
