...
class ReportController extends Controller
{
    ...
    {
        $query = LeaveRequest::with(['employee', 'region', 'district', 'ward']);
        if ($request->filled('region_id')) $query->where('region_id', $request->region_id);
        if ($request->filled('district_id')) $query->where('district_id', $request->district_id);
        if ($request->filled('ward_id')) $query->where('ward_id', $request->ward_id);
        if ($request->filled('status')) $query->where('status', $request->status);
        ...
        $regions = \App\Models\Region::orderBy('name')->get();
        return view('reports.leaves', compact('leaves', 'regions', 'stats'));
    }

    // ========== PROJECTS ==========
    public function projects(Request $request)
    {
        $query = Project::with(['organization', 'region', 'district', 'ward']);
        if ($request->filled('region_id')) $query->where('region_id', $request->region_id);
        if ($request->filled('district_id')) $query->where('district_id', $request->district_id);
        if ($request->filled('ward_id')) $query->where('ward_id', $request->ward_id);
        if ($request->filled('status')) $query->where('status', $request->status);
        ...
        $regions = \App\Models\Region::orderBy('name')->get();
        return view('reports.projects', compact('projects', 'regions', 'stats'));
    }

    // ========== TASKS ==========
    public function tasks(Request $request)
    {
        $query = Task::with(['project', 'assignee', 'region', 'district', 'ward']);
        if ($request->filled('region_id')) $query->where('region_id', $request->region_id);
        if ($request->filled('district_id')) $query->where('district_id', $request->district_id);
        if ($request->filled('ward_id')) $query->where('ward_id', $request->ward_id);
        ...
        $regions = \App\Models\Region::orderBy('name')->get();
        return view('reports.tasks', compact('tasks', 'regions', 'stats'));
    }

    // ========== PAYROLL ==========
    public function payroll(Request $request)
    {
        $query = Payslip::with(['employee', 'region', 'district', 'ward']);
        if ($request->filled('region_id')) $query->where('region_id', $request->region_id);
        if ($request->filled('district_id')) $query->where('district_id', $request->district_id);
        if ($request->filled('ward_id')) $query->where('ward_id', $request->ward_id);
        ...
        $regions = \App\Models\Region::orderBy('name')->get();
        return view('reports.payroll', compact('payslips', 'regions', 'stats'));
    }

    // ========== PAYMENT VOUCHERS ==========
    public function paymentVouchers(Request $request)
    {
        $query = PaymentVoucher::with(['project', 'region', 'district', 'ward']);
        if ($request->filled('region_id')) $query->where('region_id', $request->region_id);
        if ($request->filled('district_id')) $query->where('district_id', $request->district_id);
        if ($request->filled('ward_id')) $query->where('ward_id', $request->ward_id);
        if ($request->filled('status')) $query->where('status', $request->status);
        ...
        $regions = \App\Models\Region::orderBy('name')->get();
        return view('reports.payment-vouchers', compact('vouchers', 'regions', 'stats'));
    }

    // ========== RECEIPTS ==========
    public function receipts(Request $request)
    {
        $query = Receipt::with(['project', 'region', 'district', 'ward']);
        if ($request->filled('region_id')) $query->where('region_id', $request->region_id);
        if ($request->filled('district_id')) $query->where('district_id', $request->district_id);
        if ($request->filled('ward_id')) $query->where('ward_id', $request->ward_id);
        if ($request->filled('status')) $query->where('status', $request->status);
        ...
        $regions = \App\Models\Region::orderBy('name')->get();
        return view('reports.receipts', compact('receipts', 'regions', 'stats'));
    }

    // ========== BUDGETS ==========
    public function budgets(Request $request)
    {
        $query = Budget::with(['project', 'department', 'region', 'district', 'ward']);
        if ($request->filled('region_id')) $query->where('region_id', $request->region_id);
        if ($request->filled('district_id')) $query->where('district_id', $request->district_id);
        if ($request->filled('ward_id')) $query->where('ward_id', $request->ward_id);

        $budgets = $query->orderBy('fiscal_year', 'desc')->get();
        $regions = \App\Models\Region::orderBy('name')->get();
        $stats = [
            'total' => $budgets->count(),
        ...
        $regions = \App\Models\Region::orderBy('name')->get();
        return view('reports.budgets', compact('budgets', 'regions', 'stats'));
    }

    // ========== TRAININGS ==========
    public function trainings(Request $request)
    {
        $query = Training::withCount('enrollments')->with(['region', 'district', 'ward']);
        if ($request->filled('region_id')) $query->where('region_id', $request->region_id);
        if ($request->filled('district_id')) $query->where('district_id', $request->district_id);
        if ($request->filled('ward_id')) $query->where('ward_id', $request->ward_id);
        ...
        $regions = \App\Models\Region::orderBy('name')->get();
        return view('reports.trainings', compact('trainings', 'regions', 'stats'));
    }

    // ========== RECRUITMENT ==========
    public function recruitment(Request $request)
    {
        $query = JobPosting::withCount('applications')->with(['region', 'district', 'ward']);
        if ($request->filled('region_id')) $query->where('region_id', $request->region_id);
        if ($request->filled('district_id')) $query->where('district_id', $request->district_id);
        if ($request->filled('ward_id')) $query->where('ward_id', $request->ward_id);
    ...
    {
        $query = PerformanceReview::with(['employee', 'region', 'district', 'ward']);
        if ($request->filled('region_id')) $query->where('region_id', $request->region_id);
        if ($request->filled('district_id')) $query->where('district_id', $request->district_id);
        if ($request->filled('ward_id')) $query->where('ward_id', $request->ward_id);
        ...
        $regions = \App\Models\Region::orderBy('name')->get();
        return view('reports.performance', compact('reviews', 'regions', 'stats'));
    }

    // ========== DOCUMENTS ==========
    public function documents(Request $request)
    {
        $query = Document::with(['employee', 'project', 'region', 'district', 'ward']);
        if ($request->filled('region_id')) $query->where('region_id', $request->region_id);
        if ($request->filled('district_id')) $query->where('district_id', $request->district_id);
        if ($request->filled('ward_id')) $query->where('ward_id', $request->ward_id);
        ...
        $regions = \App\Models\Region::orderBy('name')->get();
        return view('reports.documents', compact('documents', 'regions', 'stats'));
    }
