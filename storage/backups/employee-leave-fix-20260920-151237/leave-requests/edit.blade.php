...
<div class="p-6 max-w-4xl mx-auto">
    ...
    <x-page-header title="Edit Leave Request" subtitle="{{ $leaveRequest->request_number }}" icon="fa-edit" gradient="amber" />
    ...
    <form id="form-draft" action="{{ route('leave-requests.update', $leaveRequest->id) }}" method="POST" class="bg-white rounded-2xl border border-slate-200 p-8">
        @csrf @method('PUT')
        <input type="hidden" name="action" value="draft">
        @include('leave-requests._form', ['leaveRequest' => $leaveRequest])
    </form>

    <form id="form-submit" action="{{ route('leave-requests.update', $leaveRequest->id) }}" method="POST" style="display:none;">
        @csrf @method('PUT')
    ...
    <div class="flex flex-wrap gap-3 mt-6">
        ...
        <button type="button" onclick="submitForApproval()"
            ...
            <i class="fas fa-paper-plane"></i> {{ $leaveRequest->status === 'rejected' ? 'Resubmit' : 'Submit for Approval' }}
        </button>
        <x-btn href="{{ route('leave-requests.show', $leaveRequest->id) }}" color="slate">Cancel</x-btn>
    </div>
</div>
