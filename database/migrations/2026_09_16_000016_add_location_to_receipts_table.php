...
return new class extends Migration
{
    public function up(): void
    {
        Schema::table('receipts', function (Blueprint $table) {
            if (!Schema::hasColumn('receipts', 'region_id')) {
                $table->foreignId('region_id')->nullable()->after('department_id')->constrained('regions')->nullOnDelete();
            }
            if (!Schema::hasColumn('receipts', 'district_id')) {
                $table->foreignId('district_id')->nullable()->after('region_id')->constrained('districts')->nullOnDelete();
            }
            if (!Schema::hasColumn('receipts', 'ward_id')) {
