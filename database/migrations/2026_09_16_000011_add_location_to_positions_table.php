...
return new class extends Migration
{
    public function up(): void
    {
        Schema::table('positions', function (Blueprint $table) {
            if (!Schema::hasColumn('positions', 'region_id')) {
                $table->foreignId('region_id')->nullable()->after('organizational_unit_id')->constrained('regions')->nullOnDelete();
            }
            if (!Schema::hasColumn('positions', 'district_id')) {
                $table->foreignId('district_id')->nullable()->after('region_id')->constrained('districts')->nullOnDelete();
            }
            if (!Schema::hasColumn('positions', 'ward_id')) {
