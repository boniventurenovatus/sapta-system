...
return new class extends Migration
{
    public function up(): void
    {
        Schema::table('tasks', function (Blueprint $table) {
            if (!Schema::hasColumn('tasks', 'region_id')) {
                $table->foreignId('region_id')->nullable()->after('project_id')->constrained('regions')->nullOnDelete();
            }
            if (!Schema::hasColumn('tasks', 'district_id')) {
                $table->foreignId('district_id')->nullable()->after('region_id')->constrained('districts')->nullOnDelete();
            }
            if (!Schema::hasColumn('tasks', 'ward_id')) {
